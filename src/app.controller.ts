...
export class AppController {
      ...
      if (cacheManagerAny.stores && Array.isArray(cacheManagerAny.stores)) {
        ...
        if (firstStore) {
          ...
          if (firstStore.opts && firstStore.opts.store) {
            ...
            console.log('🔧 Found via opts.store:', redisStore.constructor?.name);
          }
          
          // Pattern 2: Direct store property
          if (!redisStore && firstStore.store) {
            redisStore = firstStore.store;
            console.log('🔧 Found via store property:', redisStore.constructor?.name);
          }
          ...
          if (!redisStore) {
            ...
            for (const key of Object.keys(firstStore)) {
              ...
              if (prop && typeof prop === 'object' && prop.client) {
                console.log(`🔧 Found client in property "${key}":`, prop.constructor?.name);
                redisStore = prop;
                break;
              }
            }
  ...
  async getRedisKeys() {
    try {
      ...
      if (!cacheManagerAny.stores || !Array.isArray(cacheManagerAny.stores) || cacheManagerAny.stores.length === 0) {
        return { 
